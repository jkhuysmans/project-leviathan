namespace :openinterests_proxy do 
  ...
  and start_time > #{(date_time - 1.hour).to_i * 1e3.to_i}
  AND EXISTS (
    SELECT 1 FROM open_interests WHERE (content ->> 'timestamp')::bigint > #{(date_time - 1.hour).to_i * 1e3.to_i}
  )
  on conflict do nothing;
    SQL
    ActiveRecord::Base.connection.execute(sql)

    puts "Time spent inserting: #{Time.now - insert_start}"
