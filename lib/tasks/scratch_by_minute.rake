namespace :klines_refresh do
  ...
  task :scratch_by_minute, [:symbol, :month] => :environment do |t, args|
    ...
    def get_all_symbols
      proxy_addr = '54.74.136.212'
      proxy_port = 3128
      proxy_pass = 'CNALavjy6nuWl6bdbaB8Ug'

      url = URI('https://fapi.binance.com/fapi/v1/exchangeInfo')
      Net::HTTP.start(url.host, url.port, proxy_addr, proxy_port, nil, proxy_pass, use_ssl: url.scheme == 'https') do |http|
    ...
    all_entries.each_slice(50000) do |entries_slice|
