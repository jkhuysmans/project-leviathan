...
puts "start insert into klines"
  ...
  sql = <<-SQL
      ...
      AND EXISTS (
    SELECT 1 FROM klines WHERE (content->>0)::bigint > #{(date_time.beginning_of_month - 3.day).to_i * 1e3.to_i}
)
      on conflict do nothing;
  SQL
  ActiveRecord::Base.connection.execute(sql)

puts "finish insert into klines"
